...
async function analyseImage(dataURL) {
  ...
  const body = {
    contents : [{
      parts: [
          { text: "What is this picture?"}, //what colours are in this image? / what is the main colour of this image?
          { inline_data: {
              mime_type: "image/png",
              data: base64Data
            }}
  ...
  const geminiURL=`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`; //must change GEMINI_API_KEY in .env
